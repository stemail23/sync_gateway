...
func (doc *document) IsSGWrite(rawBody []byte) bool {
	...
	if rawBody != nil && len(rawBody) > 0 {
		if doc.syncData.IsSGWrite(doc.Cas, rawBody) {
			...
			base.LogToR("CRUD+", "Doc %s is not an SG write, based on cas and body hash. cas:%x syncCas:%q", base.UD(doc.ID), doc.Cas, doc.syncData.Cas)
			return false
		}
	}
	...
	if err != nil {
		base.WarnR("Unable to marshal doc body during SG write check for doc %s.  Error: %v", base.UD(doc.ID), err)
		return false
	}
